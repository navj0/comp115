...
def convert_american_dollars(american_dollars):
    candian_dollars=american_dollars*1.34
    r=round(candian_dollars,2)
    return r 
...
assert average({17,123})==70
assert average({66,44})==55
assert average({34,66})==50
...
assert find_max([6,9])==9
assert find_max([10,99])==99
assert find_max([1,5,6,8])==8
...
evens([2, 4, 6, 7, 8]) will returnevens([2, 4, 6, 7, 8])
...
assert sum_of_squares([4,5,10])==141
